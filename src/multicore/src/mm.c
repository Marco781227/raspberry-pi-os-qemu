...
void free_page(unsigned long p) { lock_mem(); mem_map[(p - LOW_MEMORY) / PAGE_SIZE] = 0; unlock_mem(); }
...
/* Extract table index from the virtual address and prepares a descriptor
   ...
   @shift: indicate where to find the index bits in a virtual address
   corresponding to the the target pgtable level. See project description for
   details.
...
unsigned long map_table(unsigned long *table, unsigned long shift,
  ...
  index = index & (PTRS_PER_TABLE - 1);
  if (!table[index]) { /* next level pgtable absent. alloate a new page &
                          install. */
    *new_table = 1;
...
/* map a page to the given @task at its virtual address @va.
   ...
   Descend in the task's pgtable tree and alloate any absent pgtables on the
   way.
   */
void map_page(struct task_struct *task, unsigned long va, unsigned long page) {
  unsigned long pgd;
  if (!task->mm.pgd) { /* start from the task's top-level pgtable. allocate if
                          absent */
  ...
  if (new_table) { /* we've allocated a new kernel page. take it into account
                      for future reclaim */
...
int do_mem_abort(unsigned long addr, unsigned long esr) {
  ...
  if ((dfs & 0b111100) == 0b100) {
    ...
    map_page(currents[core_id], addr & PAGE_MASK, page);
    ind++;
    if (ind > 2) {
      return -1;
    }
